...
CREATE TABLE `record` (`UserId` varchar(50) NOT NULL, `BookId` int(10) NOT NULL, `Date_of_Issue` date DEFAULT NULL, `Due_Date` date DEFAULT NULL, `Date_of_Return` date DEFAULT NULL, `Dues` int(10) DEFAULT NULL, `Renewals_left` int(10) DEFAULT NULL, `Time` time DEFAULT NULL, PRIMARY KEY (`UserId`,`BookId`), KEY `BookId` (`BookId`), CONSTRAINT `record_ibfk_1` FOREIGN KEY (`UserId`) REFERENCES `user` (`UserId`), CONSTRAINT `record_ibfk_2` FOREIGN KEY (`BookId`) REFERENCES `book` (`BookId`));
