...
CREATE TABLE `message` (`M_Id` int(10) NOT NULL AUTO_INCREMENT, `UserId` varchar(50) DEFAULT NULL, `Msg` varchar(255) DEFAULT NULL, `Date` date DEFAULT NULL, `Time` time DEFAULT NULL, PRIMARY KEY (`M_Id`), KEY `UserId` (`UserId`), CONSTRAINT `message_ibfk_1` FOREIGN KEY (`UserId`) REFERENCES `user` (`UserId`));
